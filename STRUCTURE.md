...
│   │                               shim), ExplorerMenu.cs (the folder right-click entry),
│   │                               HostReachability.cs (port 22 probe)
...
| A new settings page | the `Pages` collection in `ViewModels/SettingsViewModel.cs` + a panel in `Views/SettingsWindow.xaml` | the "Integration" page: one `SettingsPageViewModel { Title, Glyph }` entry, and a `Grid` whose `DataTrigger` on `SelectedPage.Title` matches that **exact title string** - the two are joined by nothing but the text, so a renamed page silently shows an empty panel |
| A new way the application registers itself with the system | its own file in `Services/`, called from `App.OnStartup` beside `CommandRegistration.Ensure()` | `Services/ExplorerMenu.cs` - per user under `HKCU\Software\Classes`, never machine wide; a start-time `Refresh()` that only re-points what the user already turned on; the system's own state read back as the answer, so a failed write cannot leave a switch lying |
